...
def prequential_parameters_search(
    transactions_df,
    classifier,
    input_features,
    output_feature,
    ...
    if type_search == "grid":
        ...
        parameters_search = GridSearchCV(
            pipe,
            parameters,
            scoring=scoring,
            cv=cv,
    ...
    if type_search == "random":
        ...
        parameters_search = RandomizedSearchCV(
            pipe,
            parameters,
            scoring=scoring,
            cv=cv,
...
def prequential_parameters_search_with_sample(
    transactions_df,
    classifier,
    sampler_list,
    input_features,
    ...
    if type_search == "grid":
        parameters_search = GridSearchCV(
            pipe, parameters, scoring=scoring, cv=cv, refit=False, n_jobs=n_jobs
        )
...
def model_selection_wrapper_with_sample(
    transactions_df,
    classifier,
    sampler_list,
    input_features,
...
def get_predict_proba(model, X_scaled):
    ...
    ):  # if model implement decision_function -> convert to probabilities via logistic/sigmoid
...
def get_super_learner_prediction(transformed_transaction_df, load_model):
    ...
    # if state to check if has iso_forest
    isolation_forest = load_model.get("isolation_forest", None)

    X = transformed_transaction_df[input_features].values
    X_scaled = scaler.transform(X)

    X_with_anomaly = X_scaled

    # Compute anomaly score from Isolation Forest on scaled features
    if isolation_forest is not None:
        anomaly_scores = -isolation_forest.score_samples(
            X_scaled
        )  # negative for 'higher = more anomalous'
        X_with_anomaly = np.column_stack(
            [X_scaled, anomaly_scores.reshape(-1, 1)]
        )  # Concatenate to form 16 features
    ...
    if hasattr(first_model, "n_features_in_"):
        ...
        if X_with_anomaly.shape[1] < expected_features:
            # Add dummy columns filled with 0
            dummy_cols = np.zeros(
                (X_with_anomaly.shape[0], expected_features - X_with_anomaly.shape[1])
            )
            X_with_anomaly = np.hstack([X_with_anomaly, dummy_cols])

    # Base model probabilities
    probs_list = []
    for model in base_models.values():
        prob = get_predict_proba(model, X_with_anomaly)
